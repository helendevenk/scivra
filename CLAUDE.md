...
K12 科学教育平台，面向北美高中生/教师/自学者。**实验库为核心**，111 个交互式实验横跨 5 学科（Physics/Chemistry/Biology/Earth Science/Math），课标对齐 NGSS/AP。

- **实验库**：111 个 HTML 交互式实验（73 AP Physics + 10 AP Bio + 7 AP Chem + 11 K5 + 9 Middle），全部截图 QA 验证
- **UPG（Universal Principle Generator）**：AI 实时生成零依赖 HTML 交互式可视化，Max 套餐高级功能

竞争优势：PhET 有 2-3 年技术债（Java→HTML5 迁移），Scivra 用 edu-academic 教育学术风 + 111 个实验（超 PhET 物理 66 个）+ 5 学科覆盖 + AI 生成 + 社区 UGC 做差异化。
...
- **实验库**：111 个 HTML 实验（73 AP Physics + 10 AP Bio + 7 AP Chem + 11 K5 + 9 Middle + 1 Test），全部逐个截图 QA 验证
- **实验缩略图**：66 张 Gemini API 生成，统一学术蓝风格（待补齐剩余 45 张）
...
**当前主线：Wave 9-12 课程扩展（114 → 183 个实验）**
...
|------|-----------|------|-------------------|
| PhET | ❌ | 173 个预建模拟（66 物理）+ Studio 只改预设参数 | 纯消费型，我们已有 111 个实验（超 PhET 物理数量），且覆盖 5 学科 |
...
PhET 物理 66 个模拟已全部对标覆盖（P0 27 + P1 29 + P2 10）。实际实验库规模更大：**111 个实验**横跨 5 学科。

| 学科 | 数量 | 说明 |
|------|------|------|
| AP Physics | 73 | PhET 66 对标 100% + 7 个扩展 |
| AP Biology | 10 | 细胞/遗传/生态 |
| AP Chemistry | 7 | 原子/反应/电化学 |
| Elementary K5 | 11 | 力/光/磁/能量/水循环 |
| Middle School | 9 | 牛顿/生态/板块/天气 |

验收标准：**UPG 生成效果必须不低于 PhET 对应模拟**，视觉上必须超越。全部 111 个已逐个截图 QA 验证（2026-03-23）。

...
| 数据库 | PostgreSQL + Drizzle ORM（53 张表） |
| 认证 | Better Auth + RBAC |
...
| 邮件 | Resend |
| 文档 | Fumadocs (MDX) |
| 测试 | Vitest (unit+integration, 1271 tests) + Playwright (e2e, 31 tests) |
...
│   ├── db/schema.ts                  # Drizzle schema（53 张表）
...
### 数据库
- Schema 集中在 `src/config/db/schema.ts`（53 张表，40+ 复合索引）
...
- **测试体系**：175 → 1271 tests（+626%），6 Phase 完成（T1 纯函数 → T6 E2E）
...
- 社交功能完善（评论、收藏）
- 移动端优化（QA 发现首页移动端大面积空白）
- ISR 静态化（landing/pricing/listing 页面）
- OrbitControls 去重（175 个 HTML 内联副本 → 统一引用 `public/lib/orbit-controls.js`，代码卫生）
- Gallery 空状态优化（无内容时展示引导信息）
- GitHub org 创建（`github.com/scivra`，用于公开项目展示）
- Vercel GitHub 集成修复（当前 push 不自动触发部署，需手动 `vercel deploy --prod`）

### 已完成（本轮 2026-03-31）
- ✅ 93 个 TS 类型错误修复（25 文件），`tsc --noEmit` 加入 CI 门控
- ✅ 52 张缺失实验缩略图生成（Gemini 2.5 Flash Image），覆盖率 100%（182 张 / 175 实验，含 7 张已淘汰实验的历史图）
...
### 已完成（本轮 2026-04-10）
- ✅ 自定义域名配置：scivra.com + www.scivra.com → Vercel 项目（Cloudflare DNS + TXT 验证）
- ✅ 生产 QA（Health Score 91/100）：13 页面全部 200、零控制台错误、品牌一致
- ✅ Footer GitHub 死链修复：移除不存在的 `github.com/scivra`（commit e0feff4）
- ✅ 法律页面重写：Privacy Policy / Terms of Service 全部 Scivra 化
- ✅ 博客内容清理：移除 ShipAny 模板占位文章，新增 2 篇 Scivra 原创
- ✅ Changelog / Docs Index 重写为 Scivra 内容
- ✅ 中文首页内容修复 + 实验数量同步
...
| 域名策略 | scivra.com（non-www 主域）+ www 308 重定向 | Cloudflare DNS + Vercel TXT 验证 | 2026-04-10 |
| 部署方式 | `vercel deploy --prod`（手动） | GitHub 集成未连接，push 不自动触发 | 2026-04-10 |

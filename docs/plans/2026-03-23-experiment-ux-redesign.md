...
### 十一、CTO 工程评审决策记录（2026-03-23）

| # | Issue | 决策 | 理由 |
|---|-------|------|------|
| 1 | 范围拆分 | 拆成 3 Epic（SEO / 交互 / 内容） | 独立交付、独立验证，避免大 PR |
| 2 | `standard` 字段命名冲突 | 改名为 `primaryStandard` | 与现有 `standards` 数组区分，显式 > 隐式 |
| 3 | iframe 实验门控 | postMessage 通信 + 60 秒超时降级 | 完整方案，需改 111 个 HTML 文件 |
| 4 | 301 重定向 | 砍掉，直接用新 URL | 产品未上线，无 SEO 权重需保护 |
| 5 | ExperimentClient 复杂度 | 拆出 ExperimentFlow.tsx | Flow 管理阶段状态，Client 只负责 Explore 渲染 |
| 6 | Challenge 答案安全 | 前端明文，接受风险 | 学习工具非考试，复杂度 > 风险 |
| 7 | 内容与代码分离 | Hook/Card/Challenge 文本进 i18n JSON | 天然支持 i18n，TS 文件只存结构配置 |
| 8 | 测试策略 | 按 Epic 分批写，14 路径 ~50 test cases | 跟代码一起交付，ratchet 机制自动门控 |
| 9 | LCP 优化 | Hook SSR + 后台 prefetch Three.js bundle | LCP < 1s（Hook 文字），3D 点击后基本就绪 |
| 10 | Critical gap | postMessage 必须有 60 秒超时自动解锁 | 防止漏改的 HTML 导致用户卡死在 Explore |

### 十二、设计评审决策记录（2026-03-23）

| # | 维度 | 决策 | 理由 |
|---|------|------|------|
| 1 | 首页信息层级 | 砍掉 introduce/usage/subscribe 区块，保留 Hero→FreeLabs→Features→Stats→FAQ→CTA | 减少噪音，聚焦转化 |
| 2 | Hero 设计 | 左文字 + 右实验动态 GIF/视频预览 | 首屏直接证明产品价值，避免 AI slop 模板 hero |
| 3 | 3D 加载失败降级 | 静态截图 + 参数说明表 + 保留 Learn/Challenge | 学生设备可能很差，保证学习不中断 |
| 4 | 交互状态 | 补全 7 个功能的 loading/empty/error/success/partial 状态 | 无状态定义 = 工程师随意实现 |
| 5 | 组件 token 映射 | 所有新组件映射到 theme-education.css token + 复用 shadcn 组件 | 避免重复造轮子，保持视觉一致 |
| 6 | 答错颜色 | 用 `--np-gold` 不用红色 | 教育产品不惩罚错误 |
| 7 | 手机端卡片 | 默认展开（不折叠） | 小屏点击展开每张卡片太烦人 |
| 8 | Free/Pro 标签 | Free: np-green 填充 / Pro: primary 描边 + 🔒 | 视觉快速区分，不需要读文字 |
| 9 | PaywallGate 文案 | 呼应刚完成的实验成就感 | 情感低谷时用成就感驱动转化 |
| 10 | 无障碍 | 键盘导航 + aria-label + 4.5:1 对比度 + 44px 触摸目标 + prefers-reduced-motion | 教育产品必须可访问 |

...
export const atomicInteractions: Experiment = {
  ...
  contentSections: {
    whatIsIt:
      "Pour a glass of water and you're staring at trillions of molecules pulled together just hard enough to stick but pushed apart hard enough not to collapse. Those competing forces between neutral atoms give matter its solid, liquid, and gas phases — and they're well captured by an equation Lennard-Jones wrote down in 1924. Two atoms attract weakly when far apart (van der Waals forces), reach equilibrium at a specific separation σ, and repel violently if pushed any closer (Pauli exclusion). Plot potential energy versus distance and you get a characteristic well: shallow attraction reaching down to a minimum at the bond length, then a near-vertical wall on the short side. This simulation drops atoms onto a canvas with that potential active, lets you set temperature, and shows molecules forming, vibrating, and breaking apart depending on whether their kinetic energy can climb out of the well.",

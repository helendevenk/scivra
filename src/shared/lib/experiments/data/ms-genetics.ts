...
export const msGenetics: Experiment = {
  ...
  contentSections: {
    whatIsIt:
      "Why do you look like your parents but not exactly like them? The answer is genetics — the study of how traits are passed from parents to offspring through genes. Genes are specific stretches of DNA that carry instructions for building proteins, which in turn control physical traits like eye color, hair color, or height. Most human body cells contain two copies of most genes — one inherited from each parent. Gametes (sperm and eggs) are an important exception: they carry only one copy of each gene. Sex-linked genes can also differ by sex — males have only one X chromosome, so they have just one copy of X-linked genes rather than two. These alternative versions of a gene are called alleles. When two alleles are the same (AA or aa), the organism is homozygous for that trait. When they differ (Aa), it is heterozygous. Not all alleles have equal influence: a dominant allele (written with a capital letter) masks the effect of a recessive allele (lowercase) when both are present. The Punnett square is a simple grid tool for predicting what combinations of alleles offspring are likely to receive. This simulation lets you select parent genotypes, build the Punnett square, and run a simulated breeding experiment to compare predicted ratios to actual results — including special cases like X-linked traits that behave differently in males and females.",
    ...
    teacherUseCases: [
      ...
      "Switch traitType to 1 (X-linked) as an introduction to sex-linked inheritance. Note that the simulation encodes both parents using the same AA/Aa/aa scale; discuss with students that for X-linked traits in males, a single recessive allele (represented here as Aa or aa) is expressed because there is no second X to mask it. Use this as a qualitative discussion prompt rather than a direct Punnett-square activity.",

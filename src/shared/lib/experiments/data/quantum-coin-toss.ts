...
export const quantumCoinToss: Experiment = {
  ...
  contentSections: {
    whatIsIt:
      "Schrödinger's cat is the famous teaching trick: a cat in a box that's somehow both alive and dead until you peek. Behind the metaphor is a real, concrete fact about quantum systems — they can occupy multiple states *simultaneously*, with definite probabilities for each, until something interacts with them. This simulation gives you a controllable version of that strangeness: a quantum coin whose 'heads' and 'tails' probabilities you set with a single dial called the superposition angle θ. At θ=0° the coin is definitely heads. At θ=180° it's definitely tails. At θ=90° it's an equal mixture — a real, physical superposition where the coin has no defined value until you measure. Toss it once and watch the wavefunction collapse to one definite outcome. Toss it a thousand times and watch the predicted probability distribution P(0) = cos²(θ/2) emerge. The Bloch sphere on the side gives you the geometric picture used by every working quantum computer scientist on Earth: every qubit state is a point on a unit sphere, and rotating the state moves you around it.",

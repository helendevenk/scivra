...
export const gasesIntro: Experiment = {
  ...
  contentSections: {
    whatIsIt:
      "Pump up a bike tire on a hot day and the gauge climbs even before you start riding. Open a soda in a warm car and the cap pops with extra force. Watch a balloon shrivel when you carry it from the porch into the freezer. Every one of those everyday observations is the same equation in disguise: PV = nRT. Pressure, volume, and absolute temperature of any low-pressure gas are locked together by the number of molecules and a single universal constant R = 8.314 J/(mol·K). Squeeze the container and pressure rises (Boyle's Law). Heat the gas at constant pressure and volume rises (Charles's Law). Add more molecules and pressure rises if you keep volume fixed. This lab lets you pump molecules into a transparent box, drag the wall to change volume, and click a heater to change temperature, while watching the pressure gauge respond in real time. You can hold any one of P, V, T, or n constant and see exactly what the other variables do — basically rebuilding Boyle, Charles, Gay-Lussac, and Avogadro from molecular collisions in front of your own eyes. By the end you should be able to predict the gauge reading before you touch the slider.",

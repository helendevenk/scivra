...
export const bendingLight: Experiment = {
  ...
  contentSections: {
    whatIsIt:
      "Refraction is what makes a straw look broken when you stick it in a glass of water, what makes a swimming pool seem shallower than it really is, and what lets a pair of eyeglasses bring blurry text into focus. When light crosses from one transparent material into another — air into water, water into glass, glass into diamond — its speed changes, and that speed change forces the wavefront to tilt at the boundary. Snell's Law captures the geometry exactly: n₁ sinθ₁ = n₂ sinθ₂, where n is the index of refraction and θ is measured from the normal. The simulation lets you fire a beam at any angle into materials with any pair of indices, watch the refracted ray bend toward or away from the normal, push the angle past the critical angle to trigger total internal reflection, and recover Snell's Law from raw measurements. Total internal reflection is what keeps light bouncing inside fiber optic cables and what gives diamonds their characteristic sparkle.",

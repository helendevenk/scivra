...
export const normalModes: Experiment = {
  ...
  contentSections: {
    whatIsIt:
      "Hang two pendulums next to each other and connect them with a loose spring; pluck one and walk away. A minute later the first one has gone still and the second is swinging hard — then the energy slowly trades back. That ghostly back-and-forth is what coupled oscillators do, and the cleanest way to make sense of it is normal modes. A normal mode is a special motion where every part of the system oscillates at the same frequency, all in phase or all exactly out of phase. Any messy starting condition turns out to be a sum of normal modes, each marching on its own clock. This lab lets you build a chain of one to five masses connected by springs, change the coupling strength, and excite individual modes by clicking. Watch the in-phase mode at low frequency, the anti-phase mode at higher frequency, and the beating pattern when you mix two — same idea that gives a guitar its harmonics.",

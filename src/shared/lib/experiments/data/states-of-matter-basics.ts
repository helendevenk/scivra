...
export const statesOfMatterBasics: Experiment = {
  ...
  contentSections: {
    whatIsIt:
      "An ice cube on the kitchen counter, water boiling in a kettle, dry ice fogging on a stage — three everyday demonstrations of the same idea: matter exists in different phases depending on how much kinetic energy its molecules have compared to the bonds holding them together. In a solid, molecules vibrate around fixed lattice positions, locked in place by attractive forces. Add heat and the average kinetic energy climbs (KE_avg = (3/2)k_B T) until molecules break loose from the lattice — that's melting, and the temperature pauses at the melting point while bonds rearrange. Keep heating and the molecules eventually pick up enough energy to escape the surface entirely — that's boiling, with another temperature plateau. The energy spent on those rearrangements is called latent heat, and it's why a kettle of boiling water stays at 100 °C until every last drop has vaporized. This lab lets you pick a substance, drag a temperature slider from frozen-solid to gas-phase chaos, and watch real-time at the molecular level: lattice oscillations, the chaotic flow of liquids, and the wide-open scattering of gas particles. The temperature-time graph alongside the box shows the characteristic plateau during phase transitions — a fingerprint of latent heat that you can measure directly.",

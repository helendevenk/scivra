...
export const moleculesAndLight: Experiment = {
  ...
  contentSections: {
    whatIsIt:
      "Every molecule is picky about which photons it grabs. A microwave oven heats the leftover pizza but leaves the ceramic plate cool, sunlight passes through window glass but burns your skin if it gets through ozone, and the same carbon dioxide that you exhale is the gas trapping infrared on Earth. Those everyday details all come from the same rule: a molecule absorbs an electromagnetic wave only when the photon's energy matches an allowed jump in the molecule — a rotation, a vibration, or an electronic transition. Visible photons are big enough to kick electrons into higher orbitals (color and photosynthesis), infrared photons match bond stretches and bends (heat and the greenhouse effect), and microwaves match water rotations (the kitchen oven). This lab lets you point each kind of radiation at six molecules — H₂O, CO₂, N₂, O₂, O₃, CH₄ — and watch in real time which ones light up, vibrate, or stay completely indifferent to the beam. Once you see which photon a molecule cares about, climate physics, atmospheric chemistry, and IR spectroscopy stop being separate topics — they're all the same picture.",

...
export const colorVision: Experiment = {
  ...
  contentSections: {
    whatIsIt:
      "A movie projector mixes red, green, and blue beams onto a screen and somehow conjures up sunsets, blue jeans, and skin tones — even though only three colors of light ever leave the lamp. That trick works because human eyes aren't measuring wavelengths directly. They sample light with three types of cone cells, each sensitive to a different range, and the brain reconstructs color from the ratio of their responses. That's why a phone screen can fool you into seeing yellow without ever emitting a yellow photon — it just lights up red and green pixels at the right ratio. The simulation lets you mix red, green, and blue light at any intensity and watch the resulting color appear, including the surprise that all three at full intensity make white. This is additive color mixing — the rule for screens — which works differently from subtractive mixing with paints.",

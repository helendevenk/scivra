...
export const idealGasThermodynamics: Experiment = {
  ...
  contentSections: {
    whatIsIt:
      "Almost every gas you meet — the air in a bike pump, the helium in a balloon, the steam pushing a piston — obeys the same compact rule: PV = nRT. Push the gas through different processes (heat it at constant pressure, squeeze it at constant temperature, lock the volume and warm it up, or compress it so fast no heat escapes) and the path on a pressure-volume diagram changes shape, but the gas always returns to PV = nRT at every point along the way. The PV diagram is the bookkeeping tool engineers use to design car engines, refrigerators, and rocket nozzles, because the area enclosed by a closed loop equals the net work done by the gas, and the slope of each segment tells you which state variables are changing. This lab lets you pick one of four canonical processes — isothermal, isobaric, isochoric, or adiabatic — set the starting state, and watch the gas evolve while the PV diagram updates live underneath. The First Law of Thermodynamics ΔU = Q − W stays bolted to the side of the screen so you can track exactly how heat, work, and internal energy trade off in each process. Once you can read a PV diagram fluently, the rest of thermodynamics — heat engines, refrigerators, even the atmosphere — becomes geometry.",

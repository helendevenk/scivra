...
export const bernoulliFluidDynamics: Experiment = {
  ...
  contentSections: {
    whatIsIt:
      "Hold a strip of paper just below your lower lip and blow hard across the top of it — the strip lifts. Stand near a passing subway train and feel yourself pulled toward the platform edge. Watch a curveball break across the plate or a shower curtain swing inward when you turn the water on. Every one of those surprising tugs is the same equation in action: Bernoulli's principle, the energy-conservation rule for steady, incompressible fluid flow. Bernoulli says the sum of static pressure, dynamic pressure (½ρv²), and hydrostatic pressure (ρgh) along a streamline stays constant — so where the fluid speeds up, the pressure has to drop. Pair Bernoulli with the continuity equation A₁v₁ = A₂v₂ (mass can't pile up in steady flow) and you can predict that any constriction in a pipe forces the fluid to accelerate, which then forces the static pressure to fall. That's a Venturi tube, the heart of carburetors, atomizers, fluid flowmeters, and every airplane wing's lift mechanism. This lab puts a Venturi tube on screen with adjustable inlet velocity, area ratio, fluid density, and height. Color-coded pressure gradients along the pipe make the speed-up-pressure-drop link visual, and the side-by-side numerical readouts let you check Bernoulli quantitatively at every step.",

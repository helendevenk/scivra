...
export const volcanoEruptionTypes: Experiment = {
  ...
  contentSections: {
    whatIsIt:
      "Not all volcanic eruptions look the same. Some pour out slow rivers of glowing lava that people can walk alongside, while others explode with the force of nuclear bombs and blanket entire continents in ash. The key difference is magma viscosity — basically, how thick and sticky the molten rock is. This simulation lets you change the magma type, the amount of dissolved gas trapped inside it, and the temperature, then watch the eruption style that results. Basaltic magma (low silica) flows easily and lets gas escape gently, producing wide shield volcanoes like those in Hawaii. Rhyolitic magma (high silica) is so thick that gas cannot escape — pressure builds until the whole thing explodes, creating towering ash columns and dangerous pyroclastic flows like Mt. St. Helens in 1980. Compare the shapes of the volcanoes that each eruption style builds over time.",

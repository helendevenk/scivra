...
export const waveOnString: Experiment = {
  ...
  contentSections: {
    whatIsIt:
      "Whip the end of a jump rope and a hump races to the other end and back; pluck a guitar string and a sustained tone fills the room. Each is a transverse wave on a string — the rope moves up and down, but the disturbance travels horizontally along the line. The wave's speed is set by the string itself: v = √(T/μ), where T is tension and μ is mass per unit length. Crank tension up and the wave races faster; use a heavier string and it slows down. Drive one end at a steady frequency and the wave reflects off the far end, outgoing and returning waves overlap, and at certain magic frequencies they lock into a standing wave with motionless nodes and big-amplitude antinodes. Those are the harmonics of the string, and they set the pitch of every stringed instrument on the planet.",

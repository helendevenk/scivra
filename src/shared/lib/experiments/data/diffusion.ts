...
export const diffusion: Experiment = {
  ...
  contentSections: {
    whatIsIt:
      "Open a bottle of perfume across the room and the scent reaches you in a few minutes. Drop ink into still water and the color spreads slowly outward without any stirring. Pickle a cucumber in salt and the cucumber shrivels as water leaves its cells. Every one of those everyday observations is diffusion: the net movement of particles from regions of high concentration to low concentration, driven entirely by random thermal motion of molecules. There is no clever pumping mechanism — every molecule wanders independently, but because there are more molecules on the high-concentration side, statistically more cross to the low side than the other way around. Fick's First Law captures it: J = −D · ΔC/Δx, where the diffusion flux J is proportional to the concentration gradient and D is the diffusion coefficient (which itself depends on temperature, particle mass, and the medium). Add a semi-permeable membrane that lets some molecules through but not others, and you get osmosis — the special case where the diffusing species is the solvent, usually water, crossing in response to a solute concentration difference. This lab puts a transparent two-chamber box on screen with a tunable membrane between them, lets you set temperature and starting concentrations, and shows you both the molecular-scale dance and the macroscopic concentration vs. time curve. By the time the system reaches equilibrium, you should be able to predict the timescale and the final concentration without watching another simulation.",
